...
class CKTModule(nn.Module):
    def __init__(self, channel_t, channel_s, channel_h):
        super().__init__()
        self.teacher_projectors = TeacherProjectors(channel_t, channel_h)
        self.student_projector = StudentProjector(channel_s, channel_h)
    
    def forward(self, teacher_features, student_feature):
        teacher_projected_feature = self.teacher_projectors(teacher_features)
        student_projected_feature = self.student_projector(student_feature)

        return teacher_projected_feature, student_projected_feature
...
class TeacherProjectors(nn.Module):
    ...
    **Parameters:**
    ...
    """
    def __init__(self, channel_t, channel_h):
        super().__init__()
        self.PFPs = nn.ModuleList()

        self.PFPs = nn.Sequential(
                    nn.Conv2d(in_channels=channel_t, out_channels=channel_h, kernel_size=3, stride=1, padding=1, bias=False),
                    nn.LeakyReLU(0.2),
                    nn.Conv2d(in_channels=channel_h, out_channels=channel_t, kernel_size=3, stride=1, padding=1, bias=False),
                )
    ...
    def forward(self, features):
        ...
        projected_features = self.PFPs(features)
    ...
    **Parameters:**
    ...
    def __init__(self, channel_s, channel_h):
        ...
        self.PFP = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            nn.Conv2d(in_channels=channel_h, out_channels=channel_s, kernel_size=3, stride=1, padding=1, bias=False),
        )
